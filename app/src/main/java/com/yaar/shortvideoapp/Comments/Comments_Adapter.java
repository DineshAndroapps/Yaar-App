...
public class Comments_Adapter extends RecyclerView.Adapter<Comments_Adapter.CustomViewHolder > {
    ...
    public void onBindViewHolder(final Comments_Adapter.CustomViewHolder holder, final int i) {
        ...
        if(Variables.sharedPreferences.getString(Variables.u_id,"0").equals(item.video_fb_id) || Variables.sharedPreferences.getString(Variables.u_id,"0").equals(item.fb_id)) {
            holder.imgdeletecoment.setVisibility(View.VISIBLE);

        }else {

            holder.imgdeletecoment.setVisibility(View.GONE);
        }
